...
Pada Desember 2025, kompetisi *ZeroDay Cloud* (ZDC) yang diselenggarakan oleh Wiz Research menyaksikan sebuah pengungkapan kerentanan yang mengejutkan dunia database: **bug dalam fungsi `pgp_sym_decrypt` di ekstensi `pgcrypto` PostgreSQL** yang sudah ada sejak lebih dari 20 tahun lalu. Dua peneliti, **Paul** (peneliti kerentanan di Sonar) dan **Moritz** (insinyur keamanan di Edgeless Systems), berhasil mengeksploitasi kerentanan ini untuk mendapatkan *remote code execution* (RCE) dari akun pengguna database dengan hak terbatas, membawa pulang hadiah \$30.000.
...
ZeroDay Cloud adalah kompetisi *pwn* yang mirip Pwn2Own, namun khusus untuk target perangkat lunak cloud. Peserta dapat memilih beberapa target yang diumumkan dua bulan sebelumnya, dan jika berhasil mengeksploitasi target itu di atas panggung, mereka mendapatkan imbalan finansial. Targetnya mencakup seluruh tumpukan — mulai dari kernel Linux, Docker,GitLab, hingga database seperti PostgreSQL.

### Kenapa PostgreSQL?
PostgreSQL adalah salah satu sistem database paling populer di dunia, ditulis dalam C C dengan commit terdokumentasi sejak 1996. Arsitekturnya menggunakan *multi-process*: setiap koneksi klien dilayani oleh proses backend terpisah yang mengeksekusi SQL.
...
Dalam skenario ZDC, peneliti mendapatkan akun pengguna **low-privileged default** (bukan superuser).tanpa hak khusus.

## Metodologi: Membaca Kode, Bukan Fuzzing

Paul dan Moritz mengaku mereka tidak menggunakan fuzzer canggih atau AI untuk menemukan bug ini. Malahan, mereka cukup **membaca kode sumber PostgreSQL** — metode yang sederhana tapi jarang dipakai.

> *"We discovered the bug with our own eyes and not with AI."*

Kunci temuan mereka adalah ekstensi **`pgcrypto`**. PostgreSQL memiliki 26 ekstensi *terpercaya* (*trusted*) yang bisa dimuat oleh pengguna biasa tanpa hak superuser. Ekstensi ini menjadi *attack surface* emas.
...
1. Mereka bisa mengirim *ciphertext* buatan sendiri dengan format byte = `u` (Unicode) yang sebenarnya berisi **byte arbitrer** (misal byte `0xF0` yang merupakan invalid UTF-8).
2. Karena flag Unicode aktif, PostgreSQL mencoba konversi dari UTF-8 — tapi data palsu lolos karena memang passing through *decryption* terlebih dahulu.
...
Fungsi inilah yang mereka sebut **"dirty string"** — string yang berisi byte sewenang-wenang yang bisa "mencuri" masuk lewat celah konversi encoding.
...
4. **Kompetisi seperti ZDC bernilai tinggi** — Target yang transparan (Docker Compose tersedia) memaksa peneliti berpikir kreatif, bukan mengandalkan *luck*.

## Referensi

- Presentasi: *Digging Up a 20-Year-Old Bug* — Paul & Moritz, Offensive Con 2026
- Acara: ZeroDay Cloud (ZDC) oleh Wiz Research
- Target: PostgreSQL `pgp_sym_decrypt` via ekstensi `pgcrypto`
- Reward: \$30.000 (post-auth scenario)

---

*Artikel ini dihasilkan dari transkrip otomatis. Untuk detail teknis lebih lanjut, tonton presentasi asli.*
